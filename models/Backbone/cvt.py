...
class VisionTransformer(nn.Module):
    ...
    def _init_weights_trunc_normal(self, m):
        if isinstance(m, nn.Linear):
            # print('=> init weight of Linear from trunc norm')
            trunc_normal_(m.weight, std=0.02)
            if m.bias is not None:
                # print('=> init bias of Linear to zeros')
    ...
    def _init_weights_xavier(self, m):
        if isinstance(m, nn.Linear):
            # print('=> init weight of Linear from xavier uniform')
    ...
    def init_weights(self, pretrained='', pretrained_layers=[], verbose=True):
        if os.path.isfile(pretrained):
            ...
            for k, v in pretrained_dict.items():
                ...
                if need_init:
                    if verbose:
                        print(f'=> init {k} from {pretrained}')
...
def get_cvt_models(model_size="13",
    ...
    embed_dim = msvit_spec["DIM_EMBED"]

    return msvit, embed_dim